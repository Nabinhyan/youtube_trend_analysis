...
FROM transformed.transformed_video_detail tr
         LEFT JOIN dimension.dim_video vi ON tr.video_id = vi.video_id AND
                                             tr.title = vi.title
         LEFT JOIN dimension.dim_channel ch ON tr.channel_title = ch.channel_title
         LEFT JOIN dimension.dim_date da ON tr.trending_date = da.trending_date AND
                                            tr.publish_date = da.publish_date
